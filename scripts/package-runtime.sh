...
# For static SPC builds the staging contains only bin/ (the php binary has
# extensions compiled in). Output:
...
# Smoke test before packaging. Static SPC builds must expose every catalog
# extension through `php -m`.
...
# Copy optional runtime deps (licenses, THIRD_PARTY_NOTICES, etc).
# Static builds do not include separate ext/ etc.
for dir in etc lib include metadata licenses; do
